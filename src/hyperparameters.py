...
default = SigCWGANConfig(
    mc_size=1000,
    sig_config_past=SignatureConfig(depth=2, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag(with_time=False)])),
    sig_config_future=SignatureConfig(depth=2,
                                      augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag(with_time=False)]))
    )
...
SIGCWGAN_CONFIGS = dict(
    ...
    CUSTOM=SigCWGANConfig(
        mc_size=1000,
        sig_config_past=SignatureConfig(depth=2, augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        sig_config_future=SignatureConfig(depth=2,
                                          augmentations=tuple([Scale(0.2), Cumsum(), AddLags(m=2), LeadLag()])),
        #sig_config_past=SignatureConfig(depth=3, augmentations=get_standard_augmentation(0.1)),
        #sig_config_future=SignatureConfig(depth=3, augmentations=get_standard_augmentation(0.1)),
    ),
    HSL=default,
    FMI=default,
    OMX=default,
