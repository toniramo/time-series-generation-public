...
class GAN(BaseAlgo):
    def __init__(self, base_config, gan_algo, x_real):
        ...
        self.D_steps_per_G_step = base_config.D_per_G_steps
        self.D = ResFNN(self.dim * (self.p + self.q), 1, self.hidden_dims, True).to(self.device)
        self.D_optimizer = torch.optim.Adam(self.D.parameters(), lr=self.base_config.lr_D, betas=(0, 0.9))  # Using TTUR
        self.G_optimizer = torch.optim.Adam(self.G.parameters(), lr=self.base_config.lr_G, betas=(0, 0.9))
