...
class BaseAlgo:
    ...
    def plot_losses(self):
        ...
        linewidth = 0.2 if len(self.training_loss['G_loss']) >= 10000 else 0.6
        if algo in ['RCGAN', 'TimeGAN', 'RCWGAN', 'CWGAN']:
            axes[0].plot(self.training_loss['G_loss'], label='Generator loss', linewidth=linewidth)
            if algo in ['RCGAN', 'TimeGAN']:
                axes[0].plot(self.training_loss['D_loss'], label='Discriminator loss', linewidth=linewidth)
            elif algo in ['RCWGAN','CWGAN']:
                axes[0].plot(self.training_loss['D_loss'], label='Critic loss', linewidth=linewidth)
                axes[0].plot(self.training_loss['{}_reg'.format(algo)], label='GP', linewidth=linewidth)
        elif algo == 'GMMN':
            axes[0].plot(self.training_loss['MMD'], label='MMD', linewidth=linewidth)
        elif algo == 'SigCWGAN':
        ...
        for i, test_metric in enumerate(self.test_metrics_list):
            axes[i + 1].plot(self.training_loss[test_metric.name], label=test_metric.name, linewidth=0.2)
            axes[i + 1].grid()
            axes[i + 1].legend()
