...
class Pipeline:
    ...
    def fit_transform(self, x, until=None):
        x = x.clone()
        for n, step in self.steps:
            if n == until:
                break
...
class MinMaxScaler():
    ...
    def fit(self, data):
        self.min = torch.min(data, dim=1, keepdim=True).values
        self.max = torch.max(data, dim=1, keepdim=True).values
...
def get_custom_dataset(**data_params):
    DATA_DIR = './data'
    filename = os.path.join(DATA_DIR, data_params['filename'])
    file = np.genfromtxt(filename, delimiter=",", skip_header=True)
    labels = pd.read_csv(filename, nrows=1).columns.tolist()
    
    data_raw = torch.from_numpy(file.astype(float)).float()
    data_raw.unsqueeze_(0)
    if data_params['normalizer'] == 'standard':
        pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
        data_pre = pipeline.transform(data_raw)
    else:
        print("minmax")
        pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler())])
        pipeline.fit(data_raw)
    data_pre = pipeline.transform(data_raw)
    return pipeline, data_raw, data_pre, labels
...
def get_data(data_type, p, q, **data_params):
    ...
    if data_type == 'VAR':
        ...
        pipeline, x_real_raw, x_real = get_custom_dataset(**data_params)
    elif data_type in ['HSL', 'FMI', 'OMX', 'ELEC', 'THL']:
        pipeline, x_real_raw, x_real, labels = get_custom_dataset(**data_params)
    else:
        raise NotImplementedError('Dataset %s not valid' % data_type)
    assert x_real.shape[0] == 1
    x_real = rolling_window(x_real[0], p + q)
    return pipeline, x_real_raw, x_real, labels
