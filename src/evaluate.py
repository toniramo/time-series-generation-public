...
from lib.plot import plot_summary, compare_cross_corr, visualization
...
def extract_hidden_dims(file_path):
    with open(file_path, 'r') as file:
        lines = file.readlines()

    hidden_dims = None
    for line in lines:
        if 'hidden_dims' in line:
            start_index = line.find('hidden_dims=') + len('hidden_dims=')
            end_index = line.find(')', start_index)+1
                        tuple_str = line[start_index:end_index].strip()
            hidden_dims = ast.literal_eval(tuple_str)
            break

    return hidden_dims

def extract_base_config(file_path):
    with open(file_path, 'r') as file:
        lines = file.readlines()
    base_config = None
    for line in lines:
        if 'base_config' in line:
            start_index = line.find('base_config = ') + len('base_config = ')
            end_index = line.rfind(')', start_index)+1
            base_config = eval(line[start_index:end_index])
            break

    return base_config

def evaluate_generator(model_name, seed, experiment_dir, dataset, use_cuda=True, for_optim=False, base_config=None):
    """
    ...
    if base_config == None:
        base_config = extract_base_config(os.path.join(experiment_dir, "conf.txt"))
    ...
    if for_optim: return experiment_summary
    ...
    with torch.no_grad():
        ...
        x_fake_future = G.sample(p+q, _x_past)
        plot_summary(x_fake=x_fake_future, x_real=x_real, max_lag=p+q)
    ...
    x_real_concat = torch.concat([x_past, x_future])
    
    with torch.no_grad():
        x_fake = G.sample(q, x_real_concat)
        
    visualization(x_real_concat, x_fake, "pca")
    plt.savefig(os.path.join(experiment_dir, 'pca.png'))    
    plt.close()
    
    visualization(x_real_concat, x_fake, "tsne")
    ...
    plot_summary(x_fake=x_fake, x_real=x_real, max_lag=p+q)
    plt.savefig(os.path.join(experiment_dir, 'summary_long.png'))
    plt.close()
    plt.plot(to_numpy(x_fake[0, :1000]), linewidth=0.2)
    plt.savefig(os.path.join(experiment_dir, 'long_path_1000.png'))
    plt.close()
